...
class DP {
    ...
    char[] op; //每条边的对应的操作（从1开始计数）
    int[] v; //每个顶点数值（从1开始计数）
    int[][][] m; //m[i][n][1]：第三个维度为0是最小值，为1是最大值
    int[][][] cut; //记录合并点的数组，即某条链断开的边的位置，同m用法
    Stack<Integer> s; //用栈保存合并边的顺序
    int deleteEdge; //记录最优情况下，第1条删除的边
    long highestScore; //记录最高得分
    int minf, maxf;//记录目前确定了i，j，s后的这条链的最大和最小值

    /**
     * 初始化
     **/
    public DP(int n, int[] v, char[] op, int[][][] m) {
        this.n = n;
        this.v = v;
        this.op = op;
        this.m = m;
    ...
    long polyMax() {
        ...
        int i = 1;
        while (!s.empty()) {
            System.out.println("第" + i + "次合并的边：" + s.pop());
            i++;
        }
